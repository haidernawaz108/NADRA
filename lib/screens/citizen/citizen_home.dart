...
class _CitizenDashboard extends StatelessWidget {
  ...
  Widget _buildStatusProgress(String status) {
    const statuses = [
      'Submitted', 'Under Review', 'Printed', 'Dispatched', 'Delivered'
    ];
    ...
    return Row(
      children: statuses.asMap().entries.map((e) {
        final idx = e.key;
        final isCompleted = idx <= currentIdx && status != 'Expired';
        return Expanded(
          child: Row(
            children: [
              Expanded(
              ...
              if (idx < statuses.length - 1) const SizedBox(width: 2),
            ],
          ),
        );
      }).toList(),
